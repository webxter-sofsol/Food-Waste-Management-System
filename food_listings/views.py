...
class FoodListingCreateView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        """Save listing with donor and publish within 5 seconds"""
        listing = serializer.save(donor=self.request.user)
        logger.info(f"Food listing created: {listing.id} by {self.request.user.email}")
